...
const ModernInvoiceForm = ({ navigation, route }) => {
  ...
  const handleDateChange = (event, selectedDate) => {
    ...
    if (selectedDate) {
      ...
      setDate(formatted); // Utilise une chaîne formatée pour l'état
      setFormattedDate(formatted); // Assure-toi que `formattedDate` contient la chaîne
    }
  };
        ...
        <View style={styles.inputContainer}>
          ...
          {showPicker && (
            <DateTimePicker
              ...
              mode="date" // Mode date
              display="default" // Type de picker
              onChange={(event, selectedDate) => {
                setShowPicker(false); // Masque le picker
                if (selectedDate) {
                  // Format YYYY-MM-DD
